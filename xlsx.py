...
class ExcelPlan(object):
    ...
    def __init__(self, arg):
        ...
        '''定义计划头'''
        self.title = ['层级','ERP编码','名称','模号','BOM用量','单位','初期','包装样式','包装数量','模穴类型','模穴数量','备注']
        '''定义项目'''
        ...
        for d in data :
            for li in d :
                if li[0] == '总成' :
                    ...
                    i = 0
                    for i in range(0,32):
                        sheet2.write_formula(row,col,'sumifs(生产物料推移表!%s:%s,生产物料推移表!L:L,\"计划生产\",生产物料推移表!B:B,\"%s\")'%(self.str[i+4],self.str[i+4],li[1]))
                        col += 1
